...
namespace winmaped2
{
	public partial class InputOutput
	{
		...
		public unsafe static int WriteMap_Archaic(FileInfo fi, Map map)
		{
			...
			bw.Write(map.Notes.Count);
			foreach (var note in map.Notes)
			{
				bw.Write(note.x);
				bw.Write(note.y);
				byte[] n = System.Text.Encoding.ASCII.GetBytes(note.note);
				bw.Write(n.Length);
				bw.Write(n);
			}
			...
			bw_o.Flush();
			int content_ofs = (int)bw_o.BaseStream.Position + 4; 
			bw_o.Write(content_ofs + (int)ms_outbuf2.Length);
			bw.Close();
			bw_o.Close();
